...
class DataSourceCT : public DataSource
{
public:
    /**
     * Construct
     * 
     * @param stream_from_cpu [in] stream data from CPU memory else from GPU memory
     * @param input_dir [in] source data input directory, if empty generate synthetic data
     **/
    DataSourceCT(bool stream_from_cpu, const std::string input_dir);
